...
import { EquipmentRosterModal } from './EquipmentRosterModal';
...
export const OperationsLeads: React.FC = () => {
  ...
  const [equipmentRosterTarget, setEquipmentRosterTarget] = useState<Equipment | null>(null);
  ...
  const handleAssignSubmit = async (e: React.FormEvent) => {
    ...
    if (parentLeadInstance?.events) {
       ...
       for (let index = 0; index < parentLeadInstance.events.length; index++) {
          ...
          // Duplicate equipment validation removed to prevent any hard locks
       }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {assigningOrderId && createPortal(
        <div 
          className="fixed inset-0 bg-black/80 backdrop-blur-md z-[99999] flex items-end sm:items-center justify-center p-0 md:p-4 overflow-hidden"
          onClick={(e) => { if (e.target === e.currentTarget) setAssigningOrderId(null); }}
        >
          <div id="assign_staff_modal" className="bg-zinc-900 border-x-0 border-y-0 sm:border border-zinc-800 rounded-none sm:rounded-3xl w-full h-[100dvh] sm:h-[96vh] sm:max-h-[96vh] max-w-full sm:max-w-[96vw] xl:max-w-[98vw] flex flex-col shadow-2xl relative animate-in zoom-in duration-200 overflow-hidden">
                                           ...
                                           <div className="flex flex-col sm:flex-row sm:items-start gap-2 pt-1.5 border-t border-zinc-900/50">
                                             ...
                                             <div className="flex-1 min-w-0 w-full">
                                               <EquipmentSelectorDropdown
                                                 ...
                                                 onShowRoster={setEquipmentRosterTarget}
      ...
      {equipmentRosterTarget && (
        <EquipmentRosterModal
          equipmentTarget={equipmentRosterTarget}
          onClose={() => setEquipmentRosterTarget(null)}
        />
      )}
    </div>
  );
