...
import { deserializeLeadEvents, parseQtyAndText } from '../../utils';
    ...
    const syncAssignedTeamTable = () => {
      try {
        ...
        const eventsList = ((targetProd as any)?.events && Array.isArray((targetProd as any).events) && (targetProd as any).events.length > 0)
          ...
          : (ld?.notes_special_customizations ? deserializeLeadEvents(ld.notes_special_customizations).events : [])
          || (ld?.events && Array.isArray(ld.events) && ld.events.length > 0 ? ld.events : [])
        ...
        const getEventName = (eventId?: string) => {
          if (!eventId) return '';
          const normId = cleanStr(eventId);
          const found = eventsList.find((e: any) => cleanStr(e.id) === normId || cleanStr(e.event_id) === normId);
          if (found) return found.event_name || found.custom_event_name || found.event_type || '';
          return '';
        };
        ...
        rows.forEach((row, rowIdx) => {
          ...
          const normRowDeliv = parseQtyAndText(deliverableNameCell).text.trim().toLowerCase();
          ...
          // Helper to check if assignment event matches the row's event name/id
          const eventMatches = (a: EditorAssignment) => {
            const aEvtName = cleanStr(getEventName(a.event_id));
            const aEvtId = cleanStr(a.event_id);
            
            if (!cleanEvent) {
              return !aEvtId && !aEvtName;
            }
            
            if (aEvtId && (cleanEvent === aEvtId || cleanEvent.includes(aEvtId))) return true;
            if (aEvtName && (cleanEvent === aEvtName || cleanEvent.includes(aEvtName) || aEvtName.includes(cleanEvent))) return true;
            
            // If assignment has no event_id, only match if there is exactly 1 event total
            if (!aEvtId && !aEvtName && eventsList.length <= 1) return true;

            return false;
          };

          const delivMatches = (a: EditorAssignment) => {
            const aRaw = a.speciality || a.deliverable_id || '';
            const aNorm = parseQtyAndText(aRaw).text.trim().toLowerCase();
            const aClean = cleanStr(aRaw);
            return aNorm === normRowDeliv || aClean === cleanDeliv || (normRowDeliv && aNorm.includes(normRowDeliv)) || (cleanDeliv && aClean.includes(cleanDeliv));
          };

          // Matching strategy strictly matching the EXACT production/task/deliverable assignment per event:
          ...
          matchedAssignment = availableAssignments.find(a => {
            ...
            return delivMatches(a) && (cleanStaff.includes(aStaff) || aStaff.includes(cleanStaff)) && eventMatches(a);
          });

          // 2. Deliverable AND Event match
          if (!matchedAssignment) {
            matchedAssignment = availableAssignments.find(a => delivMatches(a) && eventMatches(a));
          }

          // 3. Deliverable alone match ONLY if no other event conflict
          if (!matchedAssignment && eventsList.length <= 1) {
            matchedAssignment = availableAssignments.find(a => delivMatches(a));
          }

          // NO row index fallback to prevent cross-event contamination.
        ...
        console.error("Error in syncAssignedTeamTable:", syncErr);
      }
    };
