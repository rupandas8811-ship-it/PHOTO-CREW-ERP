...
# Extract lines 11549 to 12226 (Leads table, KPI cards, filters, and Action Dropdown)
table_slice = '\n'.join(lines[11548:12226])
...
match_iter = list(re.finditer(start_marker, table_slice))
if len(match_iter) > 0:
    start_idx = match_iter[0].start()
    end_idx = table_slice.find(end_marker, start_idx) + len(end_marker)
    ...
    table_slice = table_slice[:start_idx] + new_td + table_slice[end_idx:]

table_component = f"""{common_imports}
...
print("SalesLeadsTable.tsx generated successfully.")
