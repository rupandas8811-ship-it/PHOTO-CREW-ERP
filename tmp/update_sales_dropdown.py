...
for i, line in enumerate(lines):
    if 'if (isActionsDropdownStatus && isActiveInSales && canEdit) {' in line:
        start_idx = i
    if start_idx != -1 and 'return (' in line and 'btn_followup_' in lines[i+2] and 'View CRM' in lines[i+10]:
        end_idx = i
        break
...
if start_idx != -1 and end_idx != -1:
    new_code = """                              if (isActiveInSales && canEdit) {
                                return (
...
"""
    updated_lines = lines[:start_idx] + [new_code + '\n'] + lines[end_idx:]
