...
// Add state for selectedEquipmentStatus
...
let updated = content.replace(stateTarget, stateReplace);
...
updated = updated.replace(headerTarget, headerReplace);
...
const extractTarget = updated.substring(updated.indexOf(rowLogicTargetStart), updated.indexOf(rowLogicTargetEnd) + rowLogicTargetEnd.length);
                  ...
                  <td className="py-3 text-center">
                    {selectedEquipmentStatus.eqReceived ? (() => {
                      try {
                        ...
                        return url ? (
                           <button onClick={() => window.open(url, '_blank')} className="px-2 py-1 bg-zinc-800 hover:bg-zinc-700 text-xs rounded text-indigo-400">View Image</button>
