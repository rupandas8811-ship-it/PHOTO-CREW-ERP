...
leadlost_block = '''                              if (isLeadLostStatus) {
                                return (
...
pattern = r'if \(isLeadLostStatus\) \{.*?return \(\s*<button\s*type="button"\s*id={`btn_followup_\$\{lead\.lead_id\}`}.*?<span>View CRM</span>\s*</button>\s*\);\s*\}'
new_text, count = re.subn(pattern, leadlost_block, text, flags=re.DOTALL)
