...
const startIndex = updated.indexOf(`                                const getPhotoForType = (stageKeyword: string, typeKeyword: string) => {`);
// Find the nearest </tr> after startIndex
const endTrIndex = updated.indexOf(`                                  </tr>`, startIndex);

if (startIndex === -1 || endTrIndex === -1) {
  console.log("Could not find start or end index for row logic");
  process.exit(1);
}

const extractTarget = updated.substring(startIndex, endTrIndex);
